...
- **A per-operation timeout bounds ONE operation; a remote-triggerable blocking operation also needs a concurrency CAP to bound a FLOOD.** When a peer can make the node do something that parks a bounded shared resource (a `spawn_blocking` thread on the tokio blocking pool, a connection-pool slot, an fd budget), a timeout alone is not enough: it caps how long ONE occupies the resource, but a flood of N concurrent requests still parks N of them at once, and a blocking-pool thread can even LEAK past its timeout (`tokio::time::timeout` cancels the await, not the blocking syscall). Bound the concurrency too, with a named semaphore sized WELL BELOW the shared pool so parked work can never approach it (`RAW_STREAM_OPEN_PERMITS` for `fifo:`/`file:` opens, sibling to the per-open `RAW_STREAM_OPEN_TIMEOUT`). Refuse cleanly over the cap (`try_acquire` → a `Response::Error`), never queue unboundedly, so the flood is turned away rather than piling up pending work. The common single-request case pays nothing (one permit, briefly held). This is the availability twin of the stream/session caps (`MAX_STREAMS_PER_SESSION`, `MAX_SESSIONS`): every resource a remote peer can multiply is bounded by a named constant with a why-comment.
